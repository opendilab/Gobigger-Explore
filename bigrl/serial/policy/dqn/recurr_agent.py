        ...
        if self.total_collect_timesteps > 0 and self.total_collect_timesteps > self.learning_starts:
            ...
            for _ in range(self.update_per_collect):
                ...
                if train_sample is None:
                    ...
                    train_data = default_collate_with_dim(train_sample, dim=1)
    ...
    def update(self, train_data):
        ...
        with self.timer:
            ...
            with torch.no_grad():
                ...
                next_obs = torch.flatten(train_data['obs'][1:], start_dim=0, end_dim=1)
                ...
                if self.use_double:
                    ...
                    next_q_values = next_q_values.reshape(unroll_len, batch_size)
                else:
                    next_q_values = next_target_model_outputs['q_value'].reshape(unroll_len, batch_size, -1)
            ...
            curr_obs = torch.flatten(train_data['obs'][:-1], start_dim=0, end_dim=1)
            target_model_outputs = self.model(curr_obs)
            current_q_values = target_model_outputs['q_value'].reshape(unroll_len, batch_size, -1)
            # Retrieve the q-values for the actions from the replay buffer

            current_q_values = torch.gather(current_q_values, dim=2, index=train_data['action'].unsqueeze(-1).long()).squeeze(-1)
            # Avoid potential broadcast issue
    ...
    def collect_data(self, env_manager, ):
        ...
        for i in range(self.rollout_nstep):
            ...
            for env_id in range(env_num):
                ...
                collect_buffer[env_id]['obs'].append(self.last_obs_list[env_id])
                collect_buffer[env_id]['action'].append(actions[env_id])
                collect_buffer[env_id]['reward'].append(rewards[env_id])
                collect_buffer[env_id]['done'].append(dones[env_id])
        ...
        for env_id in range(env_num):
            collect_buffer[env_id]['obs'].append(self.last_obs_list[env_id])
            collect_data.append({
                'obs': default_collate_with_dim(collect_buffer[env_id]['obs']),
                'action': default_collate_with_dim(collect_buffer[env_id]['action']),
                'reward': default_collate_with_dim(collect_buffer[env_id]['reward']).float(),
                'done': default_collate_with_dim(collect_buffer[env_id]['done']),
            })
