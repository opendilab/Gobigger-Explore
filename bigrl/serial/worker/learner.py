...
class BaseLearner:
    ...
    def reset_stats(self):
        ...
        self.last_collect_log_show_iter = 0
        self.last_loss_show_iter = 0
        self.last_save_ckpt_iter = 0
    ...
    def run(self):
        ...
        while self.agent.total_collect_timesteps < self.total_timesteps:
            ...
            if self.agent.last_iter.val - self.last_loss_show_iter >= self.log_show_freq:
                self.last_loss_show_iter = self.agent.last_iter.val
                self.log_show()

            if self.agent.last_iter.val - self.last_save_ckpt_iter >= self.save_checkpoint_freq:
                self.last_save_ckpt_iter = self.agent.last_iter.val
                checkpoint_path = self.agent.save_checkpoint(self.checkpoint_dir)
                self.logger.info(f'save checkpoint {self.agent.last_iter.val} to {checkpoint_path}!')

            # Evaluate policy performance
            if self.agent.last_iter.val - self.last_eval_iter >= self.eval_freq:
                should_stop = self.eval_agent()
                self.last_eval_iter = self.agent.last_iter.val
                if should_stop:
                    converge_flag = True
                    break
...
if __name__ == '__main__':
    ...
    def get_args():
        ...
        parser.add_argument('--cfg', '-c', type=str, default='user_config.yaml', help='Specify path of config')
        # parser.add_argument('--seed','-s',type=int,default=0,help='Specify random seed')
    ...
    learner.run()
    # from game_zoo.atari.policy.atari_policy import atari_default_policy_config
    # from game_zoo.gctools.policy.ctools_policy import ctools_default_policy_config

    # serial_onpolicy_pipeline(cfg=atari_default_policy_config)
