...
from bigrl.core.utils import read_file, save_file
...
class CheckpointHelper(object):
    ...
    def _load_matched_model_state_dict(self, model, checkpoint_state_dict, info_print):
        r"""
        Overview:
        ...
        Arguments:
            ...
            - checkpoint_state_dict (:obj:`dict`): checkpoint's state_dict
        """
        ...
        diff['miss_keys'] = model_keys - checkpoint_keys
        diff['redundant_keys'] = checkpoint_keys - model_keys

        intersection_keys = model_keys.intersection(checkpoint_keys)
        valid_keys = []
        for k in intersection_keys:
            if model_state_dict[k].shape == checkpoint_state_dict[k].shape:
                ...
                diff['mismatch_shape_keys'].append(
                    '{}\tmodel_shape: {}\tcheckpoint_shape: {}'.format(
                        k, model_state_dict[k].shape, checkpoint_state_dict[k].shape
                    )
                )
        valid_checkpoint_state_dict = {k: v for k, v in checkpoint_state_dict.items() if k in valid_keys}
        model.load_state_dict(valid_checkpoint_state_dict, strict=False)
...
def auto_close(func: Callable) -> Callable:
    r"""
    Overview:
        Create a wrapper to wrap function, and the wrapper will call the save_checkpoint method
        whenever an exception happens.
