...
from .env_worker import EnvWorker, DummyEnvWorker, SubprocEnvWorker
...
class BaseVectorEnv(gym.Env):
    ...
    All of the VectorEnv must inherit :class:`~ctools.env.BaseVectorEnv`.
    ...
    def reset(
        ...
        # if self.is_async:
        #     self._assert_id(id)
        obs = {i: self.workers[i].reset() for i in id}
        # obs_list = [self.workers[i].reset() for i in id]
        # if isinstance(obs_list[0],np.ndarray):
        #     obs = np.stack(obs_list)
        # elif isinstance(obs_list[0],dict):
        #     keys = obs_list[0].keys()
        #     obs = {k:np.stack([obs_list[i][k] for i in id]) for k in keys }
        # elif isinstance(obs_list[0], list) and len(obs_list[0])==1 :
        #     keys = obs_list[0][0].keys()
        #     obs = {k: np.stack([obs_list[i][0][k] for i in id]) for k in keys}
        # else:
    ...
    def step(
        ...
        #         info["env_id"] = env_id
        #         result[env_id] = (obs, rew, done, info)
...
class DummyVectorEnv(BaseVectorEnv):
    ...
    def __init__(
            self, env_fns: List[Callable[[], gym.Env]], **kwargs: Any) -> None:
