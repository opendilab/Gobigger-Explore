...
class Payoff:
    ...
    Property:
        players
    """
    data_keys = ['win_rate','score', 'max_score', 'max_team_score','rank','op_score', 'op_max_score', 'op_max_team_score']
    ...
    def get_pfsp_max_score_weight(self,opponent_id):
        if opponent_id in self.stat_info_record and 'max_team_score' in self.stat_info_record[opponent_id] and \
                self.stat_info_record[opponent_id]['win_rate'].count >= self.min_win_rate_games:
            max_score = min(self.stat_info_record[opponent_id]['max_team_score'].val, 2e6 )
            max_score = max(max_score, 5e5)
            weight = (max_score - 5e5) // 1.5e6
            return weight
        else:
            return 0.5
    ...
    def get_text(self):
        table_data = []
        headers = ['opponent_id'] + self.data_keys + ['game_count']
...
if __name__ == '__main__':
    ...
    for i in range(1000):
        ...
        opponent_stat = {
            ...
            'max_score': random.random() * 1000,
            'rank':  random.random() * 1000,
        }
