...
class Features:
    def __init__(self, cfg):
        ...
        # self.max_team_num = self.cfg.agent.features.get('max_team_num', 4)
        # self.max_player_num = self.cfg.agent.features.get('max_player_num', 3)
        self.max_ball_num = self.cfg.agent.features.get('max_ball_num', 64)
        self.max_food_num = self.cfg.agent.features.get('max_food_num', 256)
        self.max_spore_num = self.cfg.agent.features.get('max_spore_num', 32)
        ...
        self.setup_action()
        self.use_value_feature = self.cfg.agent.get('use_value_feature', True)
    ...
    def _init_fake_data(self):
        ...
        self.VALUE_INFO = {
            'alliance': (torch.long, (self.max_player_num * self.max_team_num,)),
            'view_x': (torch.long, (self.max_player_num * self.max_team_num,)),
            'view_y': (torch.long, (self.max_player_num * self.max_team_num,)),  # view center (x, y) from left bottom
            'view_width': (torch.long, (self.max_player_num * self.max_team_num,)),
            'max_radius': (torch.float, (self.max_player_num * self.max_team_num,)),
            'score': (torch.long, (self.max_player_num * self.max_team_num,)),
            'team_score': (torch.long, (self.max_player_num * self.max_team_num,)),
            'team_rank': (torch.long, (self.max_player_num * self.max_team_num,)),
            'player_num': (torch.long, ()),
        }
    ...
    def get_rl_step_data(self, last=False):
        ...
        if self.use_value_feature:
            value_info = {k: torch.ones(size=v[1], dtype=v[0]) for k, v in self.VALUE_INFO.items()}
            data['obs']['value_info'] = value_info
        if not last:
    ...
    def get_marl_traj_data(self, unroll_len, player_num):
        traj_data_list = []
        for _ in range(unroll_len):
            traj_data_list.append(default_collate_with_dim([self.get_rl_step_data()['obs'] for _ in range(player_num)]))
        traj_data_list.append(default_collate_with_dim([self.get_rl_step_data(last=True)['obs'] for _ in range(player_num)]))
        traj_data = default_collate_with_dim(traj_data_list)
        return traj_data

    def get_marl_batch_data(self, unroll_len, batch_size, player_num):
        batch_data_list = []
        for _ in range(batch_size):
            batch_data_list.append(self.get_marl_traj_data(unroll_len, player_num))
        batch_data = default_collate_with_dim(batch_data_list)
        return batch_data

    def get_sl_step_data(self):
        data = {}
        scalar_info = {k: torch.ones(size=v[1], dtype=v[0]) for k, v in self.SCALAR_INFO.items()}
        team_info = {k: torch.ones(size=v[1], dtype=v[0]) for k, v in self.TEAM_INFO.items()}
        ball_info = {k: torch.ones(size=v[1], dtype=v[0]) for k, v in self.BALL_INFO.items()}
        ...
        data['action'] = torch.ones(size=(), dtype=torch.long)
        return data

    def get_sl_batch_data(self, batch_size=1):
        data = []
        for _ in range(batch_size):
            data.append(self.get_sl_step_data())
        data = default_collate_with_dim(data)
        return data
    ...
    def transform_obs(self, obs, game_player_id=1, padding=True, last_action_type=None, ):
        ...
        scalar_info = {
            ...
            'agent_id': torch.tensor(game_player_id, dtype=torch.long),
        ...
        if self.use_value_feature:
            value_obs = self.get_value_info(obs=obs,game_player_id=own_player_id)
            output_obs['value_info'] = value_obs
        return output_obs
    ...
    def get_value_info(self, obs, game_player_id=1):
        global_state, player_observations = obs
        player2team = self.get_player2team()
        own_player_id = game_player_id
        leaderboard = global_state['leaderboard']
        ...
        for game_player_id in player_observations.keys():
            ...
            game_player_rel_score = math.log((player_observations[game_player_id]['score'] + 1) / 1000) - math.log(
                (own_score + 1) / 1000)
            game_player_rel_team_score = math.log((global_state['leaderboard'][game_team_id] + 1) / 1000) - math.log(
                (own_team_score + 1) / 1000)
            game_player_rel_rank = team2rank[game_team_id] - own_rank

            all_players.append([alliance,
                                game_player_view_x,
                                game_player_view_y,
                                game_player_view_width,
                                game_player_rel_score,
                                game_player_rel_team_score,
                                game_player_rel_rank,
                                ])
        all_players = torch.as_tensor(all_players)
        player_padding_num = self.max_player_num * self.max_team_num - len(all_players)
        player_num = len(all_players)
        all_players = torch.nn.functional.pad(all_players, (0, 0, 0, player_padding_num), 'constant', 0)
        value_info = {
            'alliance': all_players[:, 0].long(),
            ...
            'view_y': all_players[:, 2].round().long(),
            'view_width': all_players[:, 3].round().long(),
            'score': all_players[:, 4].round().long().clamp_(max=10, min=-10),
            'team_score': all_players[:, 5].round().long().clamp_(max=10, min=-10),
            'team_rank': all_players[:, 6].long(),
            'player_num': torch.tensor(player_num, dtype=torch.long),
        }
        return value_info
