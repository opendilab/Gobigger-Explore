...
class SignOnehotEncoder(nn.Module):
    def __init__(self, range: int):
        super(SignOnehotEncoder, self).__init__()
        self.range = range
        self.size = 2 * self.range + 1
        self.main = nn.Embedding.from_pretrained(torch.eye(self.size), freeze=True,
                                                 padding_idx=None)

    def forward(self, x: torch.Tensor):
        x = x.long().clamp_(max=self.range,min=-self.range) + self.range
        return self.main(x)
